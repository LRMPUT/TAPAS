...
void EvaluateThread::run()
{
	...
	if(separateMarg){
		...
		marg = Inference::compMarginalsParam(curPgm,
											curLogPartFunc,
											msgs,
											paramVals,
											curObsVec);
...
//	cout << "Computing likelihood" << endl;
	for(int c = 0; c < (int)curPgm.constClusters().size(); ++c){
		const Cluster* curCluster = curPgm.constClusters()[c];
						...
						std::vector<double>& grad)
{
	...
	if(!separateMarg){
		marg = Inference::compMarginalsParam(pgm->front(),
	...
	vector<EvaluateThread*> curEval(maxThreads, NULL);
	int nextDataInst = 0;
	int numThreads = 0;
